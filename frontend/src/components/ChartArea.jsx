...
const TeamBarChart = memo(function TeamBarChart({
  ...
  const { rawSeriesMax, referenceLineValue, yAxisTicks, yAxisMax, hitRate, hitRateToneClass, graphAvg, seasonAvg, lineSummaryLabel, hasLineSummaryLabel } = useMemo(() => {
    ...
    const _hitRate = computeHitRate(displayData, normalizedBetType)
    const _seasonAvg = seasonAvgOverride != null ? seasonAvgOverride : computeSeasonAvg(displayData, normalizedBetType)
    ...
    return {
      ...
      graphAvg: computeGraphAvg(displayData, normalizedBetType),
      seasonAvg: _seasonAvg,
      lineSummaryLabel: _lineSummaryLabel,
      hasLineSummaryLabel: Boolean(_lineSummaryLabel),
    }
  }, [displayData, normalizedBetType, line, isLineType, isCorners, isDoubleChance, isBtts, isWeh, isWbh, seasonAvgOverride])

  // Multi-overlay support
  const overlayDataKeys = useMemo(() => overlayConfigs.map(cfg => `overlay_${cfg.key}`), [overlayConfigs])
  const hasOverlay = useMemo(() => overlayConfigs.length > 0 && displayData.some(d =>
    overlayDataKeys.some(dk => d[dk] != null)
  ), [overlayConfigs.length, displayData, overlayDataKeys])
  const overlayScale = useMemo(() => hasOverlay ? buildOverlayYAxisScale(displayData, overlayDataKeys) : null, [hasOverlay, displayData, overlayDataKeys])
  ...
  const barCategoryGap = computeBarCategoryGap(displayData.length)
  const barSize = computeBarSize(plotWidth, displayData.length)
      ...
      <div className="pm-chart-header">
        ...
        <div className="pm-metrics-row">
          ...
          {hasOverlay ? overlayConfigs.map(cfg => {
            ...
            const avg = Number.isFinite(rankVal) ? rankVal : computeOverlayAvg(displayData, dk)
      ...
      <div className="pm-chart-body">
        <div className="pm-chart-canvas" ref={chartCanvasRef}>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <ComposedChart data={displayData} barCategoryGap={barCategoryGap} margin={chartMargin}>
              <XAxis
                dataKey="label"
                interval={0}
                tickLine={false}
                ...
                tick={<MatchAxisTick data={displayData} containerWidth={containerWidth} />}
